...
void loop() {
// this interrupt reverses the direction, get rid of this for PI our proper directional control
  if (interruptCounter) {
  ...
  if (deltaT) {
      ...
      omegaErr = omegaDes - omegaSpeed;
      
      sum_e = sum_e + omegaErr;
      if (sum_e > IMax){
        sum_e = IMax;
      }
      else if (sum_e < -IMax){
       sum_e = -IMax;}
      D = Kp*omegaErr + Ki*(sum_e/10);
...
void plotControlData() {
  ...
  Serial.print(" ");
  Serial.print(D/10);  //PWM is scaled by 1/10 to get more intelligible graph
